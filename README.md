This repository contains scripts to download market data for options contract from "Invertir Online" API.

IOL API documentation: https://api.invertironline.com/

## CLI Usage

The tool exposes a Command Line Interface (CLI) via `src/main.py`. Below is the documentation for each supported command.

### Fetch Commands

Commands related to retrieving market data from the API.
...
#### `prices`
Lists the latest known prices for options of a specific underlying symbol.
- **Usage**: `uv run src/main.py prices <symbol>`
- **Arguments**:
  - `symbol`: The underlying symbol (e.g., GGAL).
...
Commands for managing individual trades within a strategy.
